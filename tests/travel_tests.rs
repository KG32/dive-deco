use dive_deco::{DecoModel, Depth, Supersaturation, Unit};
pub mod fixtures;

#[test]
fn travel_descent() {
    let mut model = fixtures::model_default();
    let target_depth = Depth::from_metric(40.);
...
fn travel_ascent() {
    ...
    let initial_depth = Depth::from_metric(40.);
    let bottom_time = 20 * 60;
    model.record(initial_depth, bottom_time, &air);

    let target_depth = Depth::from_metric(15.);
...
fn travel_invalid_target_depth() {
    let mut model = fixtures::model_gf((30, 70));
    model.record_travel(Depth::from_metric(-10.), 1, &fixtures::gas_air());
}
...
fn test_travel_record_with_rate() {
    let mut model = fixtures::model_default();
    let air = fixtures::gas_air();
    let initial_depth = Depth::from_metric(20.);
    let bottom_time = 20 * 60;
